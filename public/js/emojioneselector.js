(function(document, window, $) {
  ...
  function getTemplate(template, unicode, shortname) {
    return template
      ...
      .replace('{img}', emojione.imagePathPNG + unicode + '.png')
      .replace('{uni}', unicode)
  ...
  EmojiSelector.prototype.addClickEvent = function(tab, buttons) {
    var _this = this;
    buttons.on('click', function(event, data) {
      var code = $(this).find('i').data('name');
      if (_this.options.target) {
        _this.options.target.val([_this.options.target.val(), code].join(''));
        $(this).trigger('emojiClicked', {
          code: code
        });
      }
    });
  ...
  EmojiSelector.prototype.init = function() {
    var _this = this;
    var index = 0;
    $.each(this.options.filters, function(filter, params) {
      $("<i/>", {"class": "emojionselector-filter", "data-filter": filter})
        .wrapInner(shortnameTo(params.icon, _this.options.sprite ? '<i class="emojione-{uni}"/>' : '<img class="emojione" src="{img}"/>'))
        .on('click', function(event, data) {
          _this.element.find('.emojionselector-filter').removeClass('active');
          $(this).addClass('active');
          _this.element.find('.emojioneselector-tabs').children().hide().filter('.emojionselector-tab-' + filter).show();
        })
        .appendTo(_this.element.find('.emojioneselector-filters'));
      $("<div/>", {"class": "emojionselector-tab emojionselector-tab-" + filter})
        .data("items", shortnameTo(params.emoji, '<i class="emojibtn" role="button">' +
          (_this.options.sprite ? '<i class="emojione-{uni}"' : '<img class="emojione" src="{img}"') + ' data-name="{name}"/></i>'))
        .appendTo(_this.element.find('.emojioneselector-tabs'));
      index++;
    });
    _this.element.find('.emojionselector-filter').first().addClass('active');
      ...
      if (items) {
          ...
          if (_this.sprite) {
              ...
              timer = setInterval(function () {
                  for (var i = 0; i < 20 && items.length; i++) {
                      tab.append(items.shift());
                      _this.addClickEvent(_this, tab.find(".emojibtn").not(".handled").addClass("handled"));
                  }
   ...
   $.fn.emojiSelector = function(options) {
    return this.each(function() {
      if (!!this.emojiSelector) return this.emojiSelector;
      return this.emojiSelector = new EmojiSelector($(this), options);
    });
  };
